...
main_hist <- paper_hist(di_cl_df %>% filter(abandoned == 0), c("doi", "reproduction_id"), list(mean, mean)) +
    labs(title = "Distribution of Reproduction Scores: Paper Level",
             x = "Reproducibility Score",
             y = "Count")
...
```{r}
max_hist <- paper_hist(di_cl_df %>% filter(abandoned == 0), c("doi"), list(max)) +
...
```{r}
min_hist <- paper_hist(di_cl_df %>% filter(abandoned == 0), c("doi"), c(min))+
